...
parser.add_argument('data', metavar='DIR', nargs='?', default='/is/cluster/fast/jpiles/imagenet',
...
parser.add_argument('--optimizer', default='sgdm', type=str)
...
def train(train_loader, model, criterion, optimizer_state, epoch, device, args, df):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        print('trainer: Epoch {}: Train L: {:.4f}\n'.format(epoch, loss.item()))
        ...
        if args.optimizer == 'sgdm':
            with torch.no_grad(): 
                #computing moving average of gradient
                norm_avg_grad_squared = 0
                for p_idx, p in enumerate(model.parameters()):
                    if optimizer_state['avg_grad_1'][p_idx]==None:
                        optimizer_state['avg_grad_1'][p_idx] =  p.grad
                    optimizer_state['avg_grad_1'][p_idx] = args.momentum * optimizer_state['avg_grad_1'][p_idx] + (1-args.momentum)*p.grad
                    norm_avg_grad_squared = norm_avg_grad_squared + optimizer_state['avg_grad_1'][p_idx].detach().data.norm(2).item()**2
                ...
                for p_idx, p in enumerate(model.parameters()):
                    new_val = p - preconditioner * (optimizer_state['avg_grad_1'][p_idx] - args.wd * p)
                    p.copy_(new_val)
                if args.use_wandb:
            ...
            with torch.no_grad(): 
                ...
                preconditioner = sigma_curr/(1+sigma_curr*norm_grad_squared/(2*loss.item()))
                for p_idx, p in enumerate(model.parameters()):
            ...
            with torch.no_grad(): # very important
                ...
                for p_idx, p in enumerate(model.parameters()):
                    ...
                    if None in (optimizer_state['avg_grad_1'][p_idx], optimizer_state['avg_grad_2'][p_idx]): #init
                        optimizer_state['avg_grad_1'][p_idx] =  grad_p
                        optimizer_state['avg_grad_2'][p_idx] = square_grad_p
                    optimizer_state['avg_grad_2'][p_idx] = args.beta2 * optimizer_state['avg_grad_2'][p_idx] + (1-args.beta2)*square_grad_p
                    optimizer_state['avg_grad_1'][p_idx] = args.momentum * optimizer_state['avg_grad_1'][p_idx] + (1-args.momentum)*grad_p

                    #parameter update
                    preconditioner = 1/(1e-8 + optimizer_state['avg_grad_2'][p_idx].sqrt())
                    new_val = p - sigma_curr * preconditioner * optimizer_state['avg_grad_1'][p_idx]
                    p.copy_(new_val)
        else: 
