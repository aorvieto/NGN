...
def main():
    ...
    if args.seed is not None:
        ...
        warnings.warn('You have chosen to seed training (NOT REALLY!). '
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss_avg = train(train_loader, model, criterion, optimizer, epoch, device, args)
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if 0:
            if args.optimizer == 'sgdm':
                with torch.no_grad(): 
                    if args.lr_decay:
                        sigma_curr = step_decay_lr(args.lr, epoch, [15,30], 0.1)
                    else:
                        sigma_curr = args.lr
                ...
                with torch.no_grad(): 
                    ...
                    preconditioner = sigma_curr/(1+sigma_curr*grad_norm_squared/(2*loss.item()))
                    for p_idx, p in enumerate(model.parameters()):
                        new_val = p - preconditioner * (p.grad - args.wd * p)
                        p.copy_(new_val)
                    if args.use_wandb:
